...
class TestAutoPause:
    ...
    def test_should_resume_when_resources_recovered(self):
        ...
        config = ExecutionConfig(
            recovery_cpu_threshold=50.0,
            recovery_memory_threshold=60.0
        )
        auto_pause = AutoPause(config)
...
class TestResourceOptimizer:
    ...
    def test_get_optimization_suggestions(self):
        ...
        assert any("CPU" in s or "cpu" in s.lower() or "concurrent" in s.lower() for s in suggestions)
...
class TestResourceAwareExecutor:
    ...
    def test_execute_task_with_high_resources_raises(self):
        ...
        config = ExecutionConfig(
            pause_cpu_threshold=50.0,
            auto_pause_enabled=True
        )
        executor = ResourceAwareExecutor(config=config)
        
        # Mock high CPU usage
        with patch.object(executor.resource_monitor, 'get_current_metrics') as mock_metrics:
    ...
    def test_execution_state_tracking(self):
        ...
        with patch.object(executor.resource_monitor, 'get_current_metrics') as mock_metrics:
            ...
            assert executor.execution_state.degradation_count == initial_degradation_count + 1

...
from tapps_agents.mcp.tool_registry import ToolRegistry, ToolCategory
...
class TestContext7MCPServer:
    ...
    def test_resolve_library_id_with_client_dict(self):
        ...
        client_func = Mock(return_value={
            "library": "react",
            "matches": [
                {"id": "/facebook/react", "name": "React", "description": "React library"}
            ]
        })
        
        server = Context7MCPServer(
            registry=registry,
            resolve_library_client=client_func
        )
    ...
    def test_tool_parameters(self, registry):
        ...
        assert get_docs_tool.parameters["context7CompatibleLibraryID"]["required"] is True

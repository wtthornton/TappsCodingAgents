...
class TestAdaptiveCacheConfig:
    ...
    def test_check_and_adjust_disk_critical(self):
        ...
        assert config.current_settings.compression_enabled is True
        assert config.current_settings.max_cache_size_mb < config.base_profile.max_cache_size_mb
    ...
    def test_reset_to_base_profile(self):
        ...
        assert config.current_settings.max_in_memory_entries == config.base_profile.max_in_memory_entries
        assert config.current_settings.compression_enabled == config.base_profile.compression_enabled
        assert len(config.config_changes) > 0

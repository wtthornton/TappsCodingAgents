...
from tapps_agents.context7.agent_integration import Context7AgentHelper, get_context7_helper
from tapps_agents.core.config import ProjectConfig, Context7Config, Context7KnowledgeBaseConfig
...
def project_config(temp_cache_dir):
    ...
    config.context7 = Context7Config(
        enabled=True,
        knowledge_base=Context7KnowledgeBaseConfig(
            location=str(temp_cache_dir)
        )
    )
    return config
...
def helper(project_config, temp_cache_dir):
    ...
    return Context7AgentHelper(
        ...
        project_root=temp_cache_dir.parent
    )
...
class TestContext7AgentHelper:
    ...
    async def test_search_libraries_with_gateway(self, helper):
        ...
        mock_gateway.call_tool = AsyncMock(return_value={
            "success": True,
            "result": {
                "matches": [
    ...
    def test_should_use_context7_disabled(self, disabled_helper):
        ...
        result = disabled_helper.should_use_context7("I need react library documentation")

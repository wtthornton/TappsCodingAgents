...
from tapps_agents.core.hardware_profiler import HardwareProfile, CacheOptimizationProfile
...
class TestHardwareAwareRouting:
    ...
    def mock_kb_cache(self):
        ...
        cache = Mock()
        cache.get.return_value = Mock(content="test", context7_id="id", trust_score=0.9, token_count=100, cache_hits=1)
    ...
    def test_nuc_profile_optimization(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
    ...
    def test_development_profile_optimization(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
    ...
    def test_workstation_profile_optimization(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
    ...
    def test_server_profile_optimization(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
    ...
    def test_hardware_profile_detection(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
        """Test automatic hardware profile detection."""
        with patch('tapps_agents.core.unified_cache.HardwareProfiler') as mock_profiler_class:
            mock_profiler = Mock()
            mock_profiler.detect_profile.return_value = HardwareProfile.WORKSTATION
            mock_profiler.get_optimization_profile.return_value = CacheOptimizationProfile(
            ...
            mock_profiler.get_current_resource_usage.return_value = {"cpu": 30.0, "memory": 50.0}
    ...
    def test_ttl_settings_by_profile(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
        ...
        assert profile_nuc["tiered_context"]["tier1_ttl"] < profile_ws["tiered_context"]["tier1_ttl"]
        assert profile_nuc["tiered_context"]["tier2_ttl"] < profile_ws["tiered_context"]["tier2_ttl"]
        assert profile_nuc["tiered_context"]["tier3_ttl"] < profile_ws["tiered_context"]["tier3_ttl"]
    
    def test_adaptive_settings_by_profile(self, tmp_cache_root, mock_context_manager, mock_kb_cache, mock_knowledge_base):
        ...
        assert profile_nuc["adaptive"]["resource_check_interval"] < profile_ws["adaptive"]["resource_check_interval"]
        assert profile_nuc["adaptive"]["emergency_cleanup_threshold"] < profile_ws["adaptive"]["emergency_cleanup_threshold"]

...
from tapps_agents.core.best_practice_consultant import BestPracticeConsultant, BestPracticeAdvice
...
class TestLearningDecisionEngine:
    ...
    @pytest.mark.asyncio
    async def test_make_decision_high_best_practice_confidence(self, decision_engine, mock_best_practice_consultant):
        """Test decision with high best practice confidence."""
        # Mock best practice advice
        mock_advice = BestPracticeAdvice(
            advice="Use threshold 0.75",
            confidence=0.85,
            source="expert",
            cached=False
        )
        mock_best_practice_consultant.consult_best_practices = AsyncMock(return_value=mock_advice)
    ...
    async def test_make_decision_moderate_learned_confidence(self, decision_engine):
        ...
        assert decision.result.should_proceed is True
        assert "Moderate confidence from learned experience" in decision.result.reasoning
    
    @pytest.mark.asyncio
    async def test_make_decision_best_practice_fallback(self, decision_engine, mock_best_practice_consultant):
        """Test decision with best practice fallback."""
        # Mock best practice advice
        mock_advice = BestPracticeAdvice(
            advice="Use threshold 0.65",
            confidence=0.65,
            source="expert",
            cached=False
        )
    ...
    @pytest.mark.asyncio
    async def test_make_decision_default_fallback(self, decision_engine, mock_best_practice_consultant):
        """Test decision with default fallback."""
        # Mock best practice to return None (consultation failed)
        mock_best_practice_consultant.consult_best_practices = AsyncMock(return_value=None)
    ...
    def test_get_decision_statistics(self, decision_engine):
        ...
        # Make some decisions
        import asyncio
        asyncio.run(decision_engine.make_decision(
            decision_type="quality_threshold",
            learned_data={"usage_count": 10, "success_rate": 0.7, "quality_score": 0.7, "value": 0.7},
            context={},
            default_value=0.6
        ))

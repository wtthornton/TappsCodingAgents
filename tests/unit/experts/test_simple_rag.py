...
from tapps_agents.experts.simple_rag import SimpleKnowledgeBase, KnowledgeChunk
...
class TestSimpleKnowledgeBase:
    ...
    def temp_knowledge_dir(self):
        ...
        test_file.write_text("""# Home Automation Guide
...
""", encoding='utf-8')
        ...
        energy_file.write_text("""# Energy Management
...
""", encoding='utf-8')
    ...
    def test_markdown_aware_chunking(self, temp_knowledge_dir):
        ...
        kb = SimpleKnowledgeBase(temp_knowledge_dir)
        results = kb.search("protocols", max_results=2)
        
        # Headers (marked with #) should get higher scores
        # Check if any chunk starts with a header
        has_header = any(chunk.content.strip().startswith('#') for chunk in results)

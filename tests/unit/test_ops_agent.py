    ...
    with patch('tapps_agents.agents.ops.agent.load_config', return_value=mock_config):
        with patch('tapps_agents.agents.ops.agent.MAL') as mock_mal_class:
            ...
            mock_scan_response = json.dumps({
                "issues": [
                    {"severity": "high", "type": "sql_injection", "description": "Potential SQL injection", "line": 10}
                ]
            })
            mock_compliance_response = json.dumps({
                "compliance_status": "compliant",
                "checks": [{"check": "Documentation", "status": "pass", "message": "README found"}]
            })
            mock_deploy_response = json.dumps({
                "steps": [{"step": 1, "action": "Install dependencies", "command": "pip install -r requirements.txt"}],
                "rollback_steps": []
            })
...
class TestOpsAgent:
    ...
    async def test_activate(self, ops_agent):
        with patch.object(ops_agent, 'greet') as mock_greet, \
    ...
    async def test_deploy_staging(self, ops_agent, tmp_path):
        ops_agent.project_root = tmp_path
        result = await ops_agent.run("deploy", target="staging", environment="staging-env")
    ...
    async def test_infrastructure_setup_docker(self, ops_agent, tmp_path):
        ...
        result = await ops_agent.run("infrastructure-setup", infrastructure_type="docker")
    ...
    async def test_infrastructure_setup_unsupported(self, ops_agent):
        result = await ops_agent.run("infrastructure-setup", infrastructure_type="kubernetes")

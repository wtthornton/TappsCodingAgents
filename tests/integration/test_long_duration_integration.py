...
class TestLongDurationIntegration:
    ...
    def test_failure_recovery_workflow(self, long_duration_manager):
        ...
        # Start task
        long_duration_manager.start_long_duration_task(
        ...
        failure_history = long_duration_manager.failure_recovery.get_failure_history(
            task_id
        )
    ...
    def test_checkpoint_creation_on_interval(
        self, long_duration_manager, checkpoint_manager
    ):
    ...
    def test_durability_levels(self, session_manager, checkpoint_manager):
        ...
        assert (
            basic_manager.durability.checkpoint_interval
            > standard_manager.durability.checkpoint_interval
        )
        assert (
            standard_manager.durability.checkpoint_interval
            > high_manager.durability.checkpoint_interval
        )

    def test_artifact_backup_high_durability(
        self, session_manager, checkpoint_manager, temp_dir
    ):
    ...
    def test_multiple_tasks_concurrent(self, long_duration_manager):
        ...
        for i in range(3):
            ...
            long_duration_manager.start_long_duration_task(
                task_id=task_id, agent_id=f"agent-{i}", command=f"command-{i}"
            )
    ...
    def test_context_manager_cleanup(self, session_manager, checkpoint_manager):
        ...
        with LongDurationManager(
            ...
            manager.start_long_duration_task(
                task_id=task_id, agent_id="test-agent", command="test command"
            )
...
class TestFailureRecoveryScenarios:
    ...
    def test_timeout_recovery(self, recovery_setup):
        ...
        # Simulate timeout
        recovered = manager.handle_failure(
            task_id=task_id, failure_type="timeout", error_message="Operation timed out"
        )
    ...
    def test_multiple_failures_recovery(self, recovery_setup):
        ...
        recovered = manager.handle_failure(
            task_id=task_id, failure_type="crash", error_message="Another crash"
        )
        assert recovered is not None

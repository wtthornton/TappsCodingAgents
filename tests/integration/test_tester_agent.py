...
"""
    ...
    @pytest.mark.asyncio
    async def test_generate_tests_command(
        self, mock_mal, sample_code_file, tmp_path: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_tests_integration(
        self, mock_mal, sample_code_file, tmp_path: Path
    ):
        ...
        result = await agent.generate_tests_command(
            file=str(sample_code_file), integration=True
        )
    ...
    @pytest.mark.asyncio
    @patch("subprocess.run")
    async def test_run_tests_with_coverage(
        self, mock_subprocess, mock_mal, tmp_path: Path
    ):

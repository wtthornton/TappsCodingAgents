...
class TestImplementerAgent:
    ...
    def mock_reviewer(self):
        ...
        reviewer = MagicMock()
        reviewer.review_file = AsyncMock(return_value={
            "file": "test.py",
            "scoring": {
                "overall_score": 85.0
            },
    ...
    async def test_generate_code_command(self, mock_mal):
        ...
        result = await implementer.run("generate-code", specification="Create a hello function")
    ...
    async def test_implement_command_missing_file_path(self, mock_mal):
        ...
        assert "error" in result
        assert "file path" in result["error"].lower() or "file_path" in result["error"].lower()
    
    @pytest.mark.asyncio
    @patch('tapps_agents.agents.reviewer.agent.ReviewerAgent')
    async def test_implement_command_with_review_pass(self, mock_reviewer_class, mock_mal, tmp_path: Path):
        """Test implement command with review that passes."""
        # Setup mock reviewer
        mock_reviewer_instance = MagicMock()
        mock_reviewer_instance.review_file = AsyncMock(return_value={
            "file": "test.py",
            "scoring": {
                "overall_score": 85.0
            },
    ...
    @pytest.mark.asyncio
    @patch('tapps_agents.agents.reviewer.agent.ReviewerAgent')
    async def test_implement_command_with_review_fail(self, mock_reviewer_class, mock_mal, tmp_path: Path):

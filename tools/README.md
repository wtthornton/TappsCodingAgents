...
- **Run:** `tools\bd\bd.exe` (Windows) or `tools/bd/bd` (if a Unix binary is added). To use `bd` directly: `. .\scripts\set_bd_path.ps1` (session) or `.\scripts\set_bd_path.ps1 -Persist` (User PATH).

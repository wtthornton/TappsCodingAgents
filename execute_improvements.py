...
async def improve_file(file_path: str, issues: Dict[str, Any], file_index: int, total_files: int) -> Dict[str, Any]:
    ...
    from tapps_agents.core.config import load_config, MALConfig
    config = load_config()
...
"""
        ...
        log("Sending refactoring request to LLM (this may take a few minutes for large files)...", "LLM")
        ...
        try:
            ...
            elapsed = heartbeat_count[0] * 30
            log(f"Refactoring request completed successfully (took ~{elapsed}s)", "SUCCESS")
...
async def main():
    ...
    for idx, (file_path, file_data) in enumerate(files_to_improve, 1):
        ...
        if "error" in improvement:
            log(f"Improvement failed: {improvement.get('error', 'Unknown error')}", "ERROR")
            results.append({
                "file": file_path,
                "status": "failed",
        ...
        if "scoring" in verification:
            ...
            log(f"Verification failed: {verification.get('error', 'Unknown error')}", "ERROR")
            results.append({
                "file": file_path,
    ...
    if successful:
        ...
        for result in sorted(successful, key=lambda x: x.get("improvement", 0), reverse=True)[:5]:
            log(f"  {result['file']}: {result['improvement']:+.1f} points", "INFO")
    
    if failed:
        log("\nFailed files:", "WARNING")
        for result in failed:
            log(f"  {result['file']}: {result.get('error', 'Unknown error')}", "WARNING")

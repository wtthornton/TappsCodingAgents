...
def simulate_long_task(manager: LongDurationManager, task_id: str, duration_hours: float = 0.1):
    """
    ...
    for step in range(1, total_steps + 1):
        ...
        if step % 10 == 0:
            ...
            if snapshot:
                print(f"Progress: {snapshot.progress * 100:.1f}% - {snapshot.current_step}")
        ...
        if step == 50:
            ...
            try:
                ...
                if checkpoint:
                    print(f"Recovered from checkpoint at {checkpoint.progress * 100:.1f}%")
                    print(f"Resuming from step {int(checkpoint.progress * total_steps)}")
...
def example_progress_tracking():
    ...
    for entry in history:
        print(f"  {entry.timestamp.strftime('%H:%M:%S')}: {entry.progress * 100:.1f}% - {entry.current_step}")
    print()
...
def example_multiple_tasks():
    ...
    for task_id in active_tasks:
        snapshot = manager.get_progress_snapshot(task_id)
        if snapshot:
            print(f"  {task_id}: {snapshot.progress * 100:.1f}% - {snapshot.current_step}")
    print()

...
from tapps_agents.core.fallback_strategy import FallbackStrategy, TaskType
from tapps_agents.core.performance_benchmark import (
    BenchmarkResult,
...
def test_resource_monitor():
    ...
    monitor = ResourceMonitor(
        cpu_threshold=50.0, memory_threshold=70.0, disk_threshold=85.0
    )
    ...
    print(
        f"  Memory: {metrics.memory_percent:.1f}% ({metrics.memory_used_mb:.0f} MB used, {metrics.memory_available_mb:.0f} MB available)"
    )
    print(
        f"  Disk: {metrics.disk_percent:.1f}% ({metrics.disk_used_gb:.1f} GB used, {metrics.disk_free_gb:.1f} GB free)"
    )
...
def test_fallback_strategy(monitor):
    ...
    for task_name, expected_type in test_tasks:
        ...
        print(
            f"  {status} {task_name}: {task_type.value} (expected: {expected_type.value})"
        )
    ...
    for task_name, _ in test_tasks:
        ...
        bg_status = (
            "[BG] Use Background Agent"
            if decision.use_background
            else "[LOCAL] Run Local"
        )
...
def main():
    ...
    try:
        ...
        test_fallback_strategy(monitor)

        # Test 3: Performance Benchmark
        test_performance_benchmark(monitor)
        ...
        print(
            f"{'[OK]' if config_ok else '[FAIL]'} NUC Configuration: {'PASSED' if config_ok else 'FAILED'}"
        )

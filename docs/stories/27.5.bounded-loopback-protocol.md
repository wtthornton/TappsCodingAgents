# Story 27.5: Bounded Loopback Protocol
...
- RetryConfig and retry logic exists in `tapps_agents/workflow/parallel_executor.py` with max_attempts and backoff
- However, this is for step execution retries, NOT for the bounded loopback protocol
- Missing components:
  - Remediation payload structure (issue → fix instructions → implementer)
  - Deterministic re-validation after remediation
  - Issue history persistence
  - Loopback trigger on soft fails from composite gates
  - Escalation with blockers report
...
**Integration Points:**
- Composite gates: Story 27.4 output

...
class ExpertConfigModel(BaseModel):
    ...
    expert_id: str = Field(..., description="Unique expert identifier (e.g., 'expert-home-automation')")
    expert_name: str = Field(..., description="Human-readable expert name")
    primary_domain: str = Field(..., description="Domain where this expert has 51% authority")
    rag_enabled: bool = Field(default=False, description="Enable RAG for knowledge retrieval")
    fine_tuned: bool = Field(default=False, description="Use fine-tuned models (future)")
    confidence_matrix: Optional[Dict[str, float]] = Field(
        default=None,
        description="Custom confidence weights per domain (usually auto-calculated from weight matrix)"
    )
    mal_config: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional MAL-specific configuration for this expert"
    )
...
class ExpertsConfig:
    ...
    def from_yaml(cls, yaml_file: Path) -> 'ExpertsConfig':
        ...
        if not isinstance(data['experts'], list):
            raise ValueError(f"Expected 'experts' to be a list, got {type(data['experts']).__name__}")

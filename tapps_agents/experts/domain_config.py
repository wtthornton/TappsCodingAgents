...
class DomainConfig:
    ...
    def get_expert_primary_map(self) -> Dict[str, str]:
        ...
        return {
            domain.primary_expert_id: domain.name
...
class DomainConfigParser:
    ...
    def parse(domains_file: Path) -> DomainConfig:
        ...
        if not domains_file.exists():
            raise FileNotFoundError(f"Domain configuration file not found: {domains_file}")
        ...
        weight_matrix = WeightDistributor.calculate_weights(
            ...
            expert_primary_map=expert_primary_map
        )
    ...
    def _extract_domains(content: str) -> List[Domain]:
        ...
        for i in range(1, len(domain_sections), 2):
            ...
            description_points = re.findall(r'^[-*]\s*(.+?)(?:\n|$)', domain_content, re.MULTILINE)
            ...
            primary_expert_id = primary_expert_match.group(1).strip() if primary_expert_match else None
            ...
            if not primary_expert_id:
                ...
                primary_expert_id = primary_expert_match.group(0) if primary_expert_match else None
            ...
            description_points = [
                point for point in description_points
            ...
            for point in description_points:
                concept_match = re.match(r'(?:Key|Concept)[:\s]*(.+)', point, re.IGNORECASE)
                if concept_match:
    ...
    def generate_expert_config(
        ...
        output_dir: Path,
        expert_template: Optional[str] = None
    ) -> Dict[str, Path]:
        """
        ...
        weight_matrix_content = DomainConfigParser._generate_weight_matrix_yaml(domain_config)
    ...
    def _generate_weight_matrix_yaml(domain_config: DomainConfig) -> str:
        ...
        for expert_id in domain_config.get_expert_ids():
            ...
            for domain_name in domain_config.get_domain_names():
                weight = domain_config.weight_matrix.get_expert_weight(expert_id, domain_name)

...
from .domain_config import DomainConfigParser, DomainConfig
...
class ExpertSetupWizard:
    ...
    def _prompt(self, question: str, default: Optional[str] = None, choices: Optional[List[str]] = None) -> str:
    ...
    def _save_experts(self, experts: List[Dict[str, Any]]):
        ...
        config_data = {
            "# TappsCodingAgents Industry Experts Configuration": None,
            "# .tapps-agents/experts.yaml": None,
            "experts": experts
        }
        ...
        with open(self.experts_file, "w", encoding="utf-8") as f:
            ...
            yaml.dump({"experts": experts}, f, default_flow_style=False, sort_keys=False, allow_unicode=True)
    ...
    def _setup_rag_knowledge_base(self, domain: str, expert_name: str):
        ...
        if not self._prompt_yes_no(f"Enable RAG (knowledge base) for {expert_name}?", default=True):
            return False
...
"""
    ...
    def add_expert(self):
        ...
        if domains:
            ...
            if use_existing:
                domain_choice = self._prompt("Domain number or name", choices=domains + [str(i) for i in range(1, len(domains)+1)])
            ...
            if self._prompt_yes_no("Create domain entry later?", default=True):
                primary_domain = self._prompt("Primary domain name (will need to match domains.md)")
            else:
        ...
        if any(e.get("expert_id") == expert_id for e in experts):
            if not self._prompt_yes_no(f"Expert '{expert_id}' already exists. Replace?", default=False):
                print("Cancelled.")
                return
    ...
    def remove_expert(self):
        ...
        try:
            ...
            if 0 <= idx < len(experts):
                expert = experts[idx]
                if self._prompt_yes_no(f"Remove '{expert.get('expert_name')}'?", default=False):
                    experts.pop(idx)
                    self._save_experts(experts)
    ...
    def init_project(self):
        ...
        if self._prompt_yes_no("\nInitialize Cursor Rules and workflow presets?", default=True):
            ...
            if init_results["workflow_presets"]:
                print(f"  Created {len(init_results['files_created'])} workflow preset(s)")

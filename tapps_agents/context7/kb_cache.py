...
from .metadata import MetadataManager, LibraryMetadata
...
class CacheEntry:
    ...
    @classmethod
    def from_markdown(cls, library: str, topic: str, markdown_content: str) -> "CacheEntry":
        ...
        if source_line_match:
            ...
            if " (Trust Score:" in source_line:
                ...
                trust_score = float(trust_score_match.group(1)) if trust_score_match else None
            else:
        ...
        content = markdown_content[content_start:content_end].strip() if content_end > content_start else markdown_content
        ...
        return cls(
            ...
            trust_score=trust_score or (float(metadata.get("trust score", 0)) if metadata.get("trust score") and metadata.get("trust score") != "N/A" else None),
            snippet_count=snippet_count or int(metadata.get("snippet count", 0)),
            token_count=token_count or int(metadata.get("token count", 0)),
            cached_at=metadata.get("last updated") or datetime.utcnow().isoformat() + "Z",
...
class KBCache:
    ...
    def __init__(self, cache_root: Path, metadata_manager: Optional[MetadataManager] = None):
        """
        ...
        self.metadata_manager = metadata_manager or MetadataManager(self.cache_structure)
        self.cache_structure.initialize()
    
    def get(self, library: str, topic: str) -> Optional[CacheEntry]:
        """
    ...
    def delete(self, library: str, topic: Optional[str] = None) -> bool:
        """

...
class StalenessPolicy:
    ...
    def is_stale(self, last_updated: str, reference_date: Optional[datetime] = None) -> bool:
        """
    ...
    def days_until_stale(self, last_updated: str, reference_date: Optional[datetime] = None) -> int:
        """
...
class StalenessPolicyManager:
    ...
    def get_policy(self, library: str, library_type: Optional[str] = None) -> StalenessPolicy:
        """
        ...
        if any(keyword in library_lower for keyword in ["security", "auth", "jwt", "oauth", "encrypt"]):
            return self.policies["critical"]
        elif any(keyword in library_lower for keyword in ["test", "spec", "mock", "stub"]):
            return self.policies["active"]
        else:
            return self.policies.get("stable", StalenessPolicy(
    ...
    def get_refresh_recommendation(
        ...
        return {
            ...
            "recommendation": "refresh" if is_stale else "keep" if days_until_stale > 7 else "consider_refresh"
        }

...
from .hardware_profiler import HardwareProfiler, HardwareProfile, CacheOptimizationProfile
...
class AdaptiveCacheConfig:
    ...
    def __init__(
        ...
        self.base_profile = self.hardware_profiler.get_optimization_profile(self.hardware_profile)
    ...
    def _create_settings_from_profile(self, profile: CacheOptimizationProfile) -> AdaptiveCacheSettings:
    ...
    def check_and_adjust(self) -> bool:
        ...
        if metrics.memory_percent > self.memory_critical_threshold:
            ...
            if self.current_settings.max_in_memory_entries > 0:
                ...
                adjusted = True
                reasons.append(f"Critical memory pressure ({metrics.memory_percent:.1f}%)")
            ...
            if self.current_settings.max_in_memory_entries > 0:
                ...
                if self.current_settings.max_in_memory_entries != new_entries:
                    ...
                    adjusted = True
                    reasons.append(f"Reduced in-memory cache due to memory pressure ({metrics.memory_percent:.1f}%)")
        
        elif metrics.memory_percent < self.memory_low_threshold:
            # Low memory usage - restore to base profile
            if self.current_settings.max_in_memory_entries < self.base_profile.max_in_memory_entries:
                self.current_settings.max_in_memory_entries = self.base_profile.max_in_memory_entries
                self.current_settings.hybrid_mode = self.base_profile.hybrid_mode
                adjusted = True
                reasons.append(f"Restored cache settings (memory usage {metrics.memory_percent:.1f}%)")
        ...
        if metrics.cpu_percent > self.cpu_critical_threshold:
            ...
            if self.current_settings.background_indexing_enabled:
                ...
                adjusted = True
                reasons.append(f"Disabled background indexing (CPU {metrics.cpu_percent:.1f}%)")
        ...
        if disk_free_percent < self.disk_free_critical_threshold:
            ...
            if not self.current_settings.emergency_cleanup_active:
                ...
                adjusted = True
                reasons.append(f"Emergency cleanup activated (disk {disk_free_percent:.1f}% free)")
            
            # Reduce max cache size
            if self.current_settings.max_cache_size_mb > self.base_profile.max_cache_size_mb * 0.5:
                self.current_settings.max_cache_size_mb = int(self.base_profile.max_cache_size_mb * 0.5)
                adjusted = True
            ...
            if self.current_settings.max_cache_size_mb > self.base_profile.max_cache_size_mb * 0.75:
                self.current_settings.max_cache_size_mb = int(self.base_profile.max_cache_size_mb * 0.75)
                adjusted = True
                reasons.append(f"Reduced cache size (disk {disk_free_percent:.1f}% free)")
            ...
            if self.current_settings.max_cache_size_mb < self.base_profile.max_cache_size_mb:
                self.current_settings.max_cache_size_mb = self.base_profile.max_cache_size_mb
                adjusted = True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "adaptive_behavior": {
                "memory_pressure": metrics.memory_percent > self.memory_warning_threshold,

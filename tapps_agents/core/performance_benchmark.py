...
from .fallback_strategy import FallbackStrategy, TaskDecision
...
class PerformanceBenchmark:
    ...
    def benchmark_task(
        ...
        **kwargs
        ...
        start_metrics = self.resource_monitor.get_current_metrics() if self.resource_monitor else None
        ...
        if self.resource_monitor:
            ...
            avg_metrics = self.resource_monitor.get_average_metrics(duration_seconds=int(duration))
    ...
    def export_results(self, filename: Optional[str] = None):
        ...
        data = {
            ...
            "summary": {
                ...
                "avg_duration": sum(r.duration_seconds for r in self.results) / len(self.results) if self.results else 0.0,
                "avg_cpu": sum(r.cpu_avg for r in self.results) / len(self.results) if self.results else 0.0,
                "avg_memory": sum(r.memory_avg for r in self.results) / len(self.results) if self.results else 0.0
            }
        }
    ...
    def generate_report(self) -> str:
        ...
        if successful > 0:
            avg_duration = sum(r.duration_seconds for r in self.results if r.success) / successful
            avg_cpu = sum(r.cpu_avg for r in self.results if r.success) / successful
            avg_memory = sum(r.memory_avg for r in self.results if r.success) / successful
        ...
        for result in self.results:
            ...
            report.append(f"    Memory: {result.memory_avg:.1f}% (peak: {result.memory_peak:.1f}%)")
            if result.error:

...
class AgentSession:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentSession":
        ...
        data["checkpoints"] = [TaskCheckpoint.from_dict(cp) for cp in data.get("checkpoints", [])]
...
class SessionStorage:
    ...
    def list_sessions(self, state: Optional[SessionState] = None) -> List[str]:
        """
...
class SessionMonitor:
    ...
    def check_health(self, session: AgentSession) -> HealthStatus:
        ...
        if session.resource_usage:
            ...
            if session.resource_usage.memory_percent > 90.0 or session.resource_usage.cpu_percent > 85.0:
                return HealthStatus.CRITICAL
            
            # Warning if memory > 80% or CPU > 70%
            if session.resource_usage.memory_percent > 80.0 or session.resource_usage.cpu_percent > 70.0:
    ...
    def should_pause(self, session: AgentSession) -> bool:
        ...
        return (
            session.resource_usage.memory_percent > 90.0 or
            session.resource_usage.cpu_percent > 85.0
        )
...
class SessionRecovery:
    ...
    def __init__(self, session_storage: SessionStorage, checkpoint_manager: Optional[CheckpointManager] = None):
        """
    ...
    def recover_session(self, session_id: str) -> Optional[AgentSession]:
        ...
        logger.info(f"Recovered session {session_id} (duration: {session.duration_hours:.2f} hours)")
        return session
    ...
    def get_latest_checkpoint(self, session: AgentSession) -> Optional[TaskCheckpoint]:
        """
        ...
        sorted_checkpoints = sorted(
            session.checkpoints,
            key=lambda cp: cp.checkpoint_time,
            reverse=True
        )
...
class SessionManager:
    ...
    def update_session(self, session: AgentSession):
        ...
        if session.state == SessionState.ACTIVE and self.monitor.should_pause(session):
            logger.warning(f"Pausing session {session.session_id} due to resource constraints")
    ...
    def pause_session(self, session_id: str, reason: Optional[str] = None):
        """
        Pause a session.
    ...
    def resume_session(self, session_id: str) -> Optional[AgentSession]:
        """

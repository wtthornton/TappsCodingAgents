...
from .session_manager import SessionManager, SessionState, HealthStatus
...
class AutoPause:
    ...
    def should_pause(self, metrics: ResourceMetrics) -> tuple[bool, Optional[str]]:
        """
        ...
        if metrics.cpu_percent > self.config.pause_cpu_threshold:
            return True, f"CPU usage {metrics.cpu_percent:.1f}% exceeds threshold {self.config.pause_cpu_threshold}%"
        
        # Check memory
        if metrics.memory_percent > self.config.pause_memory_threshold:
            return True, f"Memory usage {metrics.memory_percent:.1f}% exceeds threshold {self.config.pause_memory_threshold}%"
        
        # Check disk
        if metrics.disk_percent > self.config.pause_disk_threshold:
            return True, f"Disk usage {metrics.disk_percent:.1f}% exceeds threshold {self.config.pause_disk_threshold}%"
    ...
    def should_resume(self, metrics: ResourceMetrics) -> bool:
        ...
        return (
            metrics.cpu_percent < self.config.recovery_cpu_threshold and
            metrics.memory_percent < self.config.recovery_memory_threshold
        )
...
class ResourceOptimizer:
    ...
    def should_degrade(self, metrics: ResourceMetrics) -> tuple[bool, Optional[str]]:
        """
        ...
        if metrics.cpu_percent > self.config.degrade_cpu_threshold:
            return True, f"CPU usage {metrics.cpu_percent:.1f}% exceeds degradation threshold {self.config.degrade_cpu_threshold}%"
        
        # Check memory
        if metrics.memory_percent > self.config.degrade_memory_threshold:
            return True, f"Memory usage {metrics.memory_percent:.1f}% exceeds degradation threshold {self.config.degrade_memory_threshold}%"
...
class ResourceAwareExecutor:
    ...
    def _check_resources(self, metrics: ResourceMetrics):
        ...
        with self._lock:
            ...
            if len(self.execution_state.resource_history) > 100:
                self.execution_state.resource_history = self.execution_state.resource_history[-100:]
            ...
            elif not should_degrade and self.execution_state.mode == ExecutionMode.DEGRADED:
    ...
    def _resume_execution(self, reason: Optional[str] = None):
    ...
    def execute(
        ...
        # Execute task
        try:
            logger.info(f"Executing task: {task_name} (mode: {self.execution_state.mode.value})")
            result = task()
            return result
        except Exception as e:

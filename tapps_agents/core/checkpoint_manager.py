...
class TaskCheckpoint:
    ...
    context: Dict[str, Any] = field(default_factory=dict)  # Agent context, intermediate results
    artifacts: List[str] = field(default_factory=list)  # Generated files, outputs
    metadata: Dict[str, Any] = field(default_factory=dict)  # Additional metadata
    checksum: Optional[str] = None  # Integrity checksum
...
class CheckpointStorage:
    ...
    def __init__(self, storage_dir: Path, hardware_profile: Optional[HardwareProfile] = None):
        """
    ...
    def save(self, checkpoint: TaskCheckpoint) -> Path:
        ...
        try:
            ...
            logger.info(f"Saved checkpoint for task {checkpoint.task_id} to {checkpoint_file}")
            return checkpoint_file
        except Exception as e:
            logger.error(f"Failed to save checkpoint for task {checkpoint.task_id}: {e}")
            raise
            ...
            if checkpoint_file.exists():
                try:
                    ...
                    if not checkpoint.validate():
                        logger.warning(f"Checkpoint integrity check failed for task {task_id}")
                        return None
                    
                    logger.info(f"Loaded checkpoint for task {task_id} from {checkpoint_file}")
                    return checkpoint
                except Exception as e:

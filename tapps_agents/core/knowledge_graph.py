...
class TaskNode:
    ...
    def get_related_tasks(self, relationship_type: Optional[RelationshipType] = None) -> List[str]:
        """
...
class KnowledgeGraph:
    ...
    def __init__(self, memory_system: Optional[TaskMemorySystem] = None):
        """
    ...
    def add_node(self, task_id: str, memory: Optional[TaskMemory] = None):
        """
    ...
    def _calculate_similarity(self, memory1: TaskMemory, memory2: TaskMemory) -> float:
        ...
        if memory1.command.lower() == memory2.command.lower():
            score += 0.4
        elif memory1.command.lower() in memory2.command.lower() or \
             memory2.command.lower() in memory1.command.lower():
            score += 0.2
        ...
        if learnings1 and learnings2:
            overlap = len(learnings1 & learnings2) / max(len(learnings1), len(learnings2))
            score += overlap * 0.3
    ...
    @classmethod
    def from_dict(cls, data: Dict, memory_system: Optional[TaskMemorySystem] = None) -> "KnowledgeGraph":
        """
...
class GraphQuery:
    ...
    def find_similar_tasks(self, task_id: str, limit: int = 5) -> List[str]:
        ...
        """
        return self.graph.get_related_tasks(
            task_id,
            RelationshipType.SIMILAR,
            limit
        )

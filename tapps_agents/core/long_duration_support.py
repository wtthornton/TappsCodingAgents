...
from .session_manager import SessionManager, AgentSession, SessionState
from .checkpoint_manager import CheckpointManager, TaskCheckpoint
from .resource_aware_executor import ResourceAwareExecutor, ExecutionMode
...
class ProgressTracker:
    ...
    def __init__(self, storage_dir: Optional[Path] = None):
        ...
        """
        self.storage_dir = Path(storage_dir) if storage_dir else Path(".tapps-agents/progress")
    ...
    def _save_snapshot(self, snapshot: ProgressSnapshot):
        ...
        snapshot_file = self.storage_dir / f"snapshot_{snapshot.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def get_progress_history(self, hours: Optional[float] = None) -> List[ProgressSnapshot]:
        """
    ...
    def backup_artifacts(self, artifacts: List[str], backup_dir: Path) -> List[Path]:
        ...
        for artifact_path in artifacts:
            ...
            try:
                ...
                logger.error(f"Failed to backup artifact {artifact}: {e}", exc_info=True)
...
class FailureRecovery:
    ...
    def recover_from_checkpoint(
        ...
        logger.info(f"Recovering task {task_id} from checkpoint at {checkpoint.checkpoint_time}")
    ...
    def get_failure_history(self, task_id: Optional[str] = None) -> List[FailureRecord]:
        """
        Get failure history.
        ...
        return [
            ...
            if f.metadata.get("task_id") == task_id
        ]
...
class LongDurationManager:
    ...
    def __init__(
        ...
        checkpoint_manager: CheckpointManager,
        resource_executor: Optional[ResourceAwareExecutor] = None,
        durability_level: DurabilityLevel = DurabilityLevel.STANDARD,
        hardware_profile: Optional[HardwareProfile] = None
    ):
        """
    ...
    def start_long_duration_task(
        ...
        logger.info(f"Started long-duration task {task_id} with session {session.session_id}")
    ...
    def update_progress(
        ...
        Args:
            ...
            metadata: Additional metadata
        """
        snapshot = self.progress_tracker.record_progress(
            progress=progress,
            current_step=current_step,
            steps_completed=steps_completed,
            total_steps=total_steps,
    ...
    def get_progress(self, task_id: str) -> Optional[ProgressSnapshot]:
        """Get latest progress for a task."""
        return self.progress_tracker.get_latest_progress()
    
    def get_progress_history(self, task_id: str, hours: Optional[float] = None) -> List[ProgressSnapshot]:
            ...
            try:
                ...
                with self._lock:
                    for task_id, session in list(self.active_tasks.items()):
                        if self.durability.should_checkpoint():
                            # Get latest progress
                            progress_snapshot = self.progress_tracker.get_latest_progress()
                            progress = progress_snapshot.progress if progress_snapshot else 0.0

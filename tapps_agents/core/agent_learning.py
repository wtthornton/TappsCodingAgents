...
from .task_memory import TaskMemorySystem, TaskMemory, TaskOutcome
...
class PatternExtractor:
    ...
    def extract_patterns(
        ...
        if not pattern_types or "function" in pattern_types:
            func_patterns = self._extract_function_patterns(code, quality_score, task_id)
        ...
        if not pattern_types or "import" in pattern_types:
            import_patterns = self._extract_import_patterns(code, quality_score, task_id)
            patterns.extend(import_patterns)
        
        # Extract structural patterns
        if not pattern_types or "structure" in pattern_types:
            struct_patterns = self._extract_structural_patterns(code, quality_score, task_id)
...
class PromptOptimizer:
    ...
    def record_test_result(
        ...
        alpha = 0.1
        variant.average_quality = alpha * quality_score + (1 - alpha) * variant.average_quality
        variant.last_tested = datetime.utcnow()
    
    def get_best_variant(self, min_tests: int = 5) -> Optional[PromptVariant]:
        """
    ...
    def optimize_for_hardware(self, prompt: str) -> str:
        ...
        if self.hardware_profile == HardwareProfile.NUC:
            ...
            lines = prompt.split("\n")
            essential_lines = [l for l in lines if not l.strip().startswith("#") or "TODO" in l]
...
class FeedbackAnalyzer:
    ...
    def correlate_prompt_changes(
        ...
        """
        correlations = {}
        for variant, score in zip(prompt_variants, quality_scores):
...
class AgentLearner:
    ...
    async def learn_from_task(
        ...
        quality_score = 0.5
        if quality_scores:
            quality_score = quality_scores.get("overall_score", 0.5) / 10.0  # Normalize to 0-1
        ...
        if code and self.learning_intensity != LearningIntensity.LOW:
            ...
            context = {
                "hardware_profile": self.hardware_profile.profile_type.value if hasattr(self.hardware_profile, 'profile_type') else "unknown",
            ...
            if decision.result.value is not None and isinstance(decision.result.value, (int, float)):
        ...
        if quality_scores:
            ...
            if not analysis.get("meets_threshold", False):
                suggestions = self.feedback_analyzer.get_improvement_suggestions(analysis)

...
class VisualFeedbackCollector:
    ...
    def get_feedback_history(self, limit: Optional[int] = None) -> List[VisualFeedback]:
        """
...
class VisualAnalyzer:
    ...
    def _analyze_layout_lightweight(self, elements: List[VisualElement]) -> LayoutMetrics:
        ...
        spacing_consistency = sum(spacing_scores) / len(spacing_scores) if spacing_scores else 0.5
    ...
    def _analyze_layout_detailed(
        ...
        spacing_consistency = sum(spacing_scores) / len(spacing_scores) if spacing_scores else 0.7
        
        # Alignment score
        x_positions = [elem.position[0] for elem in elements]
        y_positions = [elem.position[1] for elem in elements]
        
        x_alignment = 1.0 - (max(x_positions) - min(x_positions)) / 1000.0 if x_positions else 0.5
        y_alignment = 1.0 - (max(y_positions) - min(y_positions)) / 1000.0 if y_positions else 0.5
    ...
    def analyze_accessibility(
        ...
        if html_content:
            aria_labels_present = "aria-label" in html_content or "aria-labelledby" in html_content
        
        # Check for focus indicators
        focus_indicators_present = False
        if html_content:
            focus_indicators_present = ":focus" in html_content or "focus-visible" in html_content
    ...
    def calculate_quality_score(
        ...
        quality_score = (
            layout_score * weights.get("layout", 0.6) +
...
class UIComparator:
    ...
    def compare_iterations(
        ...
        if current.quality_score > previous.quality_score:
            improvements.append(f"Quality score improved from {previous.quality_score:.2f} to {current.quality_score:.2f}")
        elif current.quality_score < previous.quality_score:
            regressions.append(f"Quality score decreased from {previous.quality_score:.2f} to {current.quality_score:.2f}")
        else:
        ...
        if len(current.issues) < len(previous.issues):
            improvements.append(f"Reduced issues from {len(previous.issues)} to {len(current.issues)}")
        elif len(current.issues) > len(previous.issues):
            regressions.append(f"Increased issues from {len(previous.issues)} to {len(current.issues)}")
    ...
    def learn_from_feedback(self, feedback: VisualFeedback):
        ...
        if feedback.layout_metrics:
            ...
            if layout.spacing_consistency > 0.8:
                self.patterns["high_spacing_consistency"] = self.patterns.get("high_spacing_consistency", 0) + 1
            if layout.alignment_score > 0.8:
                self.patterns["high_alignment"] = self.patterns.get("high_alignment", 0) + 1
        ...
        if feedback.accessibility_metrics:
            ...
            if acc.color_contrast_score > 0.8:
                self.patterns["good_contrast"] = self.patterns.get("good_contrast", 0) + 1
            if acc.keyboard_navigable:
                self.patterns["keyboard_navigable"] = self.patterns.get("keyboard_navigable", 0) + 1

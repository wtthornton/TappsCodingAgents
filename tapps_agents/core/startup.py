...
from ..context7.refresh_queue import RefreshQueue
from ..context7.staleness_policies import StalenessPolicyManager
...
async def refresh_stale_documentation(
    ...
    if not config.context7 or not config.context7.enabled:
        return {
            "success": False,
            "message": "Context7 is not enabled",
            "refreshed": 0
        }
    ...
    try:
        ...
        if background:
            ...
            asyncio.create_task(_process_refresh_queue_background(context7_commands, max_refresh))
            return {
                "success": True,
            ...
            refreshed = await _process_refresh_queue_sync(context7_commands, max_refresh)
            return {
                "success": True,
...
async def _process_refresh_queue_background(
    ...
    try:
        ...
        if result.get("success"):
            ...
            if refreshed > 0:
                print(f"[Startup] Refreshed {refreshed} documentation entries in background", file=sys.stderr)
    except Exception as e:

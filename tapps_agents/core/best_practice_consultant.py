...
class BestPracticeAdvice:
    ...
    cached: bool = False
    consultation_result: Optional[Any] = None  # ConsultationResult if from expert
...
class BestPracticeConsultant:
    ...
    async def consult_best_practices(
        ...
        logger.debug(f"Cache miss for decision type: {decision_type}, consulting expert")
    ...
    def _generate_query(self, decision_type: str, context: Dict[str, Any]) -> str:
        ...
        if not template:
            # Fallback template
            template = (
                "What are the best practices for {decision_type}? "
                "Context: {context}"
            )
        ...
        try:
            ...
            logger.warning(f"Missing context key {e} for decision type {decision_type}, using fallback")
            return template.format(
                decision_type=decision_type,

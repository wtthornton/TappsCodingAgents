...
class ArtifactValidator:
    ...
    @staticmethod
    def validate_artifacts(artifacts: List[str], project_root: Optional[Path] = None) -> Dict[str, bool]:
        """
    ...
    @staticmethod
    def all_artifacts_exist(artifacts: List[str], project_root: Optional[Path] = None) -> bool:
        """
        ...
        validation_results = ArtifactValidator.validate_artifacts(artifacts, project_root)
...
class ResumeHandler:
    ...
    def can_resume(self, task_id: str) -> tuple[bool, Optional[str]]:
        """
        ...
        # Validate artifacts
        if not self.artifact_validator.all_artifacts_exist(checkpoint.artifacts, self.project_root):
            missing = [
                artifact for artifact, exists in
                self.artifact_validator.validate_artifacts(checkpoint.artifacts, self.project_root).items()
                if not exists
            ]

...
class TieredContextBuilder:
    ...
    def build_context(
        ...
        if "function_signatures" in config.includes or config.inherits:
            context["content"]["functions"] = self._build_function_signatures(module_info)
        ...
        if tier in (ContextTier.TIER2, ContextTier.TIER3):
            if "function_bodies" in config.includes:
                context["content"]["function_bodies"] = self._build_function_bodies(file_path, module_info)
            
            if "local_dependencies" in config.includes and include_related:
                context["content"]["dependencies"] = self._find_local_dependencies(file_path, module_info)
        ...
        if tier == ContextTier.TIER3:
            ...
            try:
                ...
                context["content"]["full_file"] = self._truncate_to_tokens(full_content, config.max_tokens)
            except Exception:
                pass
    ...
    def _build_function_signatures(self, module_info: ModuleInfo) -> List[Dict[str, Any]]:
        ...
        return [
            {
                ...
                "docstring": func.docstring[:200] if func.docstring else None  # Truncate long docstrings
            }
    ...
    def _build_function_bodies(self, file_path: Path, module_info: ModuleInfo) -> Dict[str, str]:
    ...
    def _find_local_dependencies(self, file_path: Path, module_info: ModuleInfo) -> List[str]:

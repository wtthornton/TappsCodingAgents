...
from .detector import ProjectDetector, ProjectCharacteristics, WorkflowTrack
...
class WorkflowRecommender:
    ...
    def recommend(
        ...
        if auto_load and workflow_file:
            ...
            if workflow_path.exists():
                try:
                    ...
                    workflow_file = self._find_best_match(workflow_file, available_workflows)
                    if workflow_file:
                        workflow_path = self.workflows_dir / f"{workflow_file}.yaml"
                        if workflow_path.exists():
                            workflow = WorkflowParser.parse_file(workflow_path)
        ...
        alternative_workflows = [
            w for w in available_workflows 
            if w != workflow_file
        ]
        
        # Generate recommendation message
        message = self._generate_message(characteristics, workflow_file, workflow is not None)
    ...
    def _find_best_match(self, preferred: str, available: List[str]) -> Optional[str]:
        ...
        for workflow in available:
            if preferred_lower in workflow.lower() or workflow.lower() in preferred_lower:
                return workflow

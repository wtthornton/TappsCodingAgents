...
class ImproverAgent(BaseAgent):
    ...
    def __init__(self, mal: Optional[MAL] = None, config: Optional[ProjectConfig] = None):
        super().__init__(agent_id="improver", agent_name="Improver Agent", config=config)
        if config is None:
            config = load_config()
        self.config = config
    ...
    def greet(self):
        print(f"Hello! I am the {self.agent_name}. I help refactor and improve code quality.")
    ...
    async def _handle_refactor(self, file_path: Optional[str] = None, instruction: Optional[str] = None, **kwargs: Any) -> Dict[str, Any]:
        ...
        instruction_text = instruction or "Improve code structure, readability, and maintainability while preserving functionality."
...
Return only the refactored code, wrapped in ```python code blocks."""
    ...
    async def _handle_optimize(self, file_path: Optional[str] = None, optimization_type: str = "performance", **kwargs: Any) -> Dict[str, Any]:
        """Optimize code for performance, memory, or other metrics."""
        if not file_path:
            return {"error": "File path required. Usage: *optimize <file_path> [optimization_type]"}
        ...
        optimization_instruction = optimization_prompts.get(optimization_type, optimization_prompts["performance"])
...
Return only the optimized code, wrapped in ```python code blocks."""
    ...
    async def _handle_improve_quality(self, file_path: Optional[str] = None, **kwargs: Any) -> Dict[str, Any]:

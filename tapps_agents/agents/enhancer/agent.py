...
class EnhancerAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[ProjectConfig] = None):
        super().__init__(agent_id="enhancer", agent_name="Enhancer Agent", config=config)
        if config is None:
            config = load_config()
        self.config = config
    ...
    def get_commands(self) -> List[Dict[str, str]]:
        ...
        return base_commands + [
            {"command": "*enhance", "description": "Full enhancement pipeline (all stages)"},
            {"command": "*enhance-quick", "description": "Quick enhancement (stages 1-3 only)"},
            {"command": "*enhance-stage", "description": "Run specific enhancement stage"},
            {"command": "*enhance-resume", "description": "Resume interrupted enhancement session"},
        ]
    ...
    async def run(self, command: str, **kwargs: Any) -> Dict[str, Any]:
        ...
        if command == "help":
            ...
            return await self._enhance_full(prompt, output_format, output_file, config_path)
    ...
    async def _enhance_quick(
        ...
        try:
            ...
            return {
                ...
                "session_id": session_id,
                "enhanced_prompt": result,
                "stages_completed": ["analysis", "requirements", "architecture", "synthesis"]
            }
        
        except Exception as e:
            return {"error": f"Quick enhancement failed: {str(e)}", "session_id": session_id}
    ...
    async def _enhance_stage(
        ...
        if stage not in stage_handlers:
            return {"error": f"Unknown stage: {stage}. Available: {list(stage_handlers.keys())}"}
        ...
        try:
            ...
            if stage == "requirements":
                ...
                result = await handler(prompt, session["stages"].get("requirements", {}))
            elif stage == "codebase_context":
                result = await handler(prompt, session["stages"].get("analysis", {}))
            elif stage == "quality":
                result = await handler(prompt, session["stages"].get("requirements", {}))
            elif stage == "implementation":
                result = await handler(prompt, session["stages"].get("requirements", {}), session["stages"].get("architecture", {}))
            ...
            return {"error": f"Stage {stage} failed: {str(e)}", "session_id": session_id}
...
Provide structured JSON response."""
    ...
    async def _stage_requirements(
        ...
        requirements = requirements_result.get("requirements", {}) if "error" not in requirements_result else {}
            ...
            for domain in domains:
                try:
                    ...
                    expert_consultations[domain] = {
                        ...
                        "sources": [r.get("sources", []) for r in consultation.responses]
                    }
                except Exception:
    ...
    async def _stage_quality(
        ...
        return {
            ...
            "testing_requirements": ["Unit tests required", "Integration tests for APIs"],
...
Create a comprehensive, context-aware enhanced prompt that includes all relevant information."""
    ...
    def _load_enhancement_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _create_session(self, prompt: str, config: Dict[str, Any]) -> str:
        ...
        session_id = hashlib.md5(f"{prompt}{datetime.now().isoformat()}".encode()).hexdigest()[:8]
    ...
    def _create_markdown_from_stages(self, session: Dict[str, Any]) -> str:
        ...
        if "requirements" in stages:
            ...
            if reqs.get("expert_consultations"):
                ...
                for domain, consultation in reqs["expert_consultations"].items():
                    ...
                    lines.append(f"**Confidence**: {consultation.get('confidence', 0):.2%}")
                    lines.append(f"**Agreement**: {consultation.get('agreement_level', 0):.2%}")
                    lines.append(f"**Primary Expert**: {consultation.get('primary_expert', 'unknown')}")
                    lines.append("")
        ...
        if "quality" in stages:
            ...
            lines.append(f"**Overall Score Threshold**: {quality.get('code_quality_thresholds', {}).get('overall_score', 70)}")
            lines.append("")

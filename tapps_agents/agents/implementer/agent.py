...
class ImplementerAgent(BaseAgent):
    ...
    def __init__(self, mal: Optional[MAL] = None, config: Optional[ProjectConfig] = None):
        ...
        implementer_config = config.agents.implementer if config and config.agents else None
        self.require_review = implementer_config.require_review if implementer_config else True
    ...
    def get_commands(self) -> List[Dict[str, str]]:
        ...
        return base_commands + [
            {"command": "*implement", "description": "Generate and write code to file (with review)"},
            {"command": "*generate-code", "description": "Generate code from specification (no file write)"},
    ...
    async def run(self, command: str, **kwargs) -> Dict[str, Any]:
        ...
        if command == "help":
            ...
            if not specification:
                return {"error": "Specification required. Usage: *implement <specification> <file_path>"}
            if not file_path:
                return {"error": "File path required. Usage: *implement <specification> <file_path>"}
            
            context = kwargs.get("context")
            language = kwargs.get("language", "python")
            return await self.implement(specification, file_path, context=context, language=language)
        
        elif command == "generate-code":
            specification = kwargs.get("specification") or kwargs.get("text", "")
            if not specification:
                return {"error": "Specification required. Usage: *generate-code <specification> [--file=<file_path>]"}
            
            file_path = kwargs.get("file_path") or kwargs.get("file")
            context = kwargs.get("context")
            language = kwargs.get("language", "python")
            return await self.generate_code(specification, file_path=file_path, context=context, language=language)
        
        elif command == "refactor":
            file_path = kwargs.get("file_path") or kwargs.get("file")
            instruction = kwargs.get("instruction") or kwargs.get("text", "")
            if not file_path:
                return {"error": "File path required. Usage: *refactor <file_path> <instruction>"}
            if not instruction:
                return {"error": "Instruction required. Usage: *refactor <file_path> <instruction>"}
    ...
    async def _review_code(self, code: str, file_path: Path) -> Optional[Dict[str, Any]]:

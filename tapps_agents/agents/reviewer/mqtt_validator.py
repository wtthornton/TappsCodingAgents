...
class MQTTValidator:
    ...
    def validate_connection_pattern(self, code: str) -> dict[str, Any]:
        ...
        if "mqtt.Client" in code and "on_connect" not in code:
            ...
            suggestions.append("Implement on_connect callback to handle connection events")
        ...
        if "mqtt" in code.lower() and "reconnect" not in code.lower():
            ...
            suggestions.append("Implement automatic reconnection with reconnect_delay_set()")
        ...
        if "publish" in code or "subscribe" in code:
            ...
            if "subscribe" in code and "#" in code:
                suggestions.append("Be careful with multi-level wildcard (#) - can receive many messages")
            
            # Check for topic naming
            topics = re.findall(r'(?:publish|subscribe)\(["\']([^"\']+)["\']', code)
            for topic in topics:
                if len(topic.split("/")) > 5:
                    suggestions.append(f"Topic '{topic}' is deeply nested - consider flattening")
        ...
        if "mqtt.Client" in code and "will_set" not in code:
            suggestions.append("Consider setting will message for unexpected disconnects")

...
class TypeScriptScorer:
    ...
    def score_file(self, file_path: Path, code: str) -> Dict[str, Any]:
        ...
        scores["maintainability_score"] = self._calculate_maintainability(code, file_path)
    ...
    def _calculate_type_checking_score(self, file_path: Path) -> float:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                error_count = result.stderr.count("error TS") + result.stdout.count("error TS")
    ...
    def _calculate_maintainability(self, code: str, file_path: Optional[Path] = None) -> float:
        """
        ...
        try:
            ...
            has_comments = sum(1 for line in lines if line.strip().startswith("//") or "/*" in line)
            has_javadoc = sum(1 for line in lines if "/**" in line or "* @" in line)
            has_types = sum(1 for line in lines if ": " in line and ("string" in line or "number" in line or "boolean" in line))
    ...
    def get_eslint_issues(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"available": True, "issues": [], "error": "Failed to parse ESLint output"}

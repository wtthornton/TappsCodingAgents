...
from ...core.config import ProjectConfig, load_config, PlannerAgentConfig
...
class PlannerAgent(BaseAgent):
    ...
    def get_commands(self) -> List[Dict[str, str]]:
        ...
        return base_commands + [
            {"command": "*plan", "description": "Create a plan for a feature/requirement"},
            {"command": "*create-story", "description": "Generate a user story from description"},
            {"command": "*list-stories", "description": "List all stories in the project"},
        ]
    ...
    async def run(self, command: str, **kwargs) -> Dict[str, Any]:
        ...
        if command == "help":
            ...
            description = kwargs.get("description") or kwargs.get("text", "")
            if not description:
                return {"error": "Description required. Usage: *create-story <description> [--epic=<epic>] [--priority=<priority>]"}
...
Format your response as structured text."""
    ...
    async def create_story(self, description: str, epic: Optional[str] = None, priority: str = "medium") -> Dict[str, Any]:
        """
        ...
        # Create story file
        story_file = await self._write_story_file(story_metadata, acceptance_criteria, tasks)
    ...
    async def list_stories(self, epic: Optional[str] = None, status: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        story_files = list(self.stories_dir.glob("story-*.md")) + list(self.stories_dir.glob("story-*.yaml"))
    ...
    def _generate_story_id(self, description: str, epic: Optional[str] = None) -> str:
    ...
    def _infer_domain(self, description: str) -> str:
        ...
        if any(word in description_lower for word in ["api", "endpoint", "service"]):
            return "backend"
        elif any(word in description_lower for word in ["ui", "interface", "component", "page"]):
            return "frontend"
        elif any(word in description_lower for word in ["test", "testing", "spec"]):
            return "testing"
        elif any(word in description_lower for word in ["documentation", "docs", "guide"]):
...
Respond with ONLY a single number (1-5)."""
    ...
    async def _generate_story_details(self, description: str) -> tuple[List[str], List[str]]:
        """Generate acceptance criteria and tasks using LLM."""
...
2. Task 2"""
        ...
        try:
            ...
            return acceptance_criteria or ["Story implementation meets requirements"], tasks or ["Implement story requirements"]
        except Exception as e:
    ...
    async def _write_story_file(self, metadata: Dict[str, Any], acceptance_criteria: List[str], tasks: List[str]) -> Path:

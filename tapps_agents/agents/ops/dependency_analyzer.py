...
class DependencyAnalyzer:
    ...
    def get_dependency_tree(self) -> Optional[Dict[str, Any]]:
        """
            ...
            if format_type == "json":
                try:
                    ...
                    for vuln in raw_vulns:
                        ...
                        severity = vuln.get("severity", vuln.get("severity_level", "unknown")).lower()
                        ...
                        if self._severity_meets_threshold(severity, severity_threshold):
                            vulnerabilities.append({
                                ...
                                "installed_version": vuln.get("installed_version", vuln.get("version", "unknown")),
                                "vulnerability_id": vuln.get("id", vuln.get("vulnerability_id", "unknown")),
                                "severity": severity,
                                "description": vuln.get("description", vuln.get("summary", "")),
                                "fixed_versions": vuln.get("fixed_versions", vuln.get("fix_versions", [])),
                                "cve_id": vuln.get("aliases", [{}])[0].get("CVE", "") if isinstance(vuln.get("aliases"), list) and vuln.get("aliases") else None
                            })
                    ...
                    if result.stdout:
                        vulnerabilities = self._parse_text_audit_output(result.stdout, severity_threshold)
    ...
    def check_outdated(self) -> Optional[Dict[str, Any]]:
        """
    ...
    def _count_packages(self, tree_json: Any) -> int:
        ...
        if isinstance(tree_json, dict):
            ...
            if "package" in tree_json:
                ...
                # List of packages
                for item in tree_json.values() if hasattr(tree_json, "values") else [tree_json]:
    ...
    def _parse_text_audit_output(self, text: str, threshold: str) -> List[Dict[str, Any]]:
            ...
            if "VULN" in line.upper() or "CVE" in line.upper():
                ...
                if len(parts) >= 2:
                    vulnerabilities.append({
                        ...
                        "vulnerability_id": parts[1] if len(parts) > 1 else "unknown",
                        "severity": "unknown",
                        "description": line
                    })

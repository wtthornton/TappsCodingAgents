...
from ...core.visual_feedback import (
    ...
    VisualFeedback, VisualElement, LayoutMetrics, AccessibilityMetrics, VisualElementType
)
...
class IterativeRefinement:
    ...
    def __init__(
        ...
        self.feedback_collector = VisualFeedbackCollector(hardware_profile=self.hardware_profile)
        ...
        while iteration < self.config.max_iterations:
            ...
            if self.config.enable_layout_analysis:
                layout_metrics = self.visual_analyzer.analyze_layout(
                    visual_elements,
                    design_spec=requirements.get("design_spec")
                )
            ...
            result = IterationResult(
                ...
                should_continue=self._should_continue_iteration(quality_score, improvements, iteration)
            )
            ...
            if not result.should_continue:
                logger.info(f"Stopping refinement. Quality threshold met or max iterations reached.")
                break
    ...
    def _extract_visual_elements(self, html_content: str) -> List[VisualElement]:
        ...
        text_count = html_content.count("<p") + html_content.count("<h1") + html_content.count("<h2")
    ...
    def get_refinement_summary(self) -> Dict[str, Any]:
        ...
        initial_quality = self.iteration_history[0].quality_score if len(self.iteration_history) > 1 else final_result.quality_score
...
class VisualDesignerAgent(DesignerAgent):
    ...
    def get_commands(self) -> List[Dict[str, str]]:
        ...
        return base_commands + [
            {"command": "*visual-design", "description": "Design UI with visual feedback and iterative refinement"}
        ]

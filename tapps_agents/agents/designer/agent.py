...
class DesignerAgent(BaseAgent, ExpertSupportMixin):
    ...
    def __init__(self, mal: Optional[MAL] = None, config: Optional[ProjectConfig] = None):
        super().__init__(agent_id="designer", agent_name="Designer Agent", config=config)
        if config is None:
            config = load_config()
        self.config = config
    ...
    def get_commands(self) -> List[Dict[str, str]]:
        ...
        return base_commands + [
            {"command": "*design-api", "description": "Design API contracts and endpoints"},
            {"command": "*design-data-model", "description": "Design data models and schemas"},
            {"command": "*design-ui", "description": "Design UI/UX specifications"},
            {"command": "*create-wireframe", "description": "Create wireframe (text-based)"},
            {"command": "*define-design-system", "description": "Define design system (colors, typography, components)"},
        ]
    ...
    async def run(self, command: str, **kwargs: Any) -> Dict[str, Any]:
        ...
        if command == "help":
            ...
            return await self._create_wireframe(screen_description, wireframe_type, output_file)
            ...
            return await self._define_design_system(project_description, brand_guidelines, output_file)

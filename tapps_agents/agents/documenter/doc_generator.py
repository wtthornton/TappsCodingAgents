...
class DocGenerator:
    ...
    def _build_api_docs_prompt(
        ...
        if output_format == "markdown":
            prompt_parts.append("Format as Markdown with headers (##), code blocks, and lists.")
    ...
    def _build_docstring_prompt(
        ...
        """Build prompt for docstring generation."""
        functions_without_docs = [f["name"] for f in structure["functions"] if not f["has_docstring"]]
        classes_without_docs = [c["name"] for c in structure["classes"] if not c["has_docstring"]]

...
class GitMCPServer:
    ...
    def _register_tools(self):
        ...
        self.registry.register(
            ...
            parameters={
                "repo_path": {"type": "string", "required": False, "description": "Repository path"},
                "limit": {"type": "integer", "required": False, "description": "Number of commits"}
            },
            cache_strategy="none"
        )
    ...
    def _run_git_command(self, args: list, repo_path: Optional[Path] = None) -> str:
        """Run a Git command."""
        cmd = ["git"] + args
    ...
    def git_diff(self, file_path: Optional[str] = None, repo_path: Optional[str] = None) -> Dict[str, Any]:
        """Get Git diff."""
        path = Path(repo_path) if repo_path else Path.cwd()
        args = ["diff"]
    ...
    def git_log(self, repo_path: Optional[str] = None, limit: int = 10) -> Dict[str, Any]:
        """Get Git log."""
        path = Path(repo_path) if repo_path else Path.cwd()
    ...
    def git_blame(self, file_path: str, repo_path: Optional[str] = None) -> Dict[str, Any]:
        """Get Git blame."""
        path = Path(repo_path) if repo_path else Path.cwd()
